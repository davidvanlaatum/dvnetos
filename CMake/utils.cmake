...
function(set_target_warnings target)
  get_target_property(TARGET_WARNINGS_DONE ${target} TARGET_WARNINGS_DONE)
  if (NOT TARGET_WARNINGS_DONE)
    set(WARN "$<TARGET_FILE:${target}>.link.warnings.json;$<LIST:TRANSFORM,$<TARGET_OBJECTS:${target}>,APPEND,.compile.warnings.json>;$<LIST:TRANSFORM,$<TARGET_OBJECTS:${target}>,APPEND,.clang-tidy.warnings.json>")
    get_target_property(ADDITIONAL_CLEAN_FILES ${target} ADDITIONAL_CLEAN_FILES)
    if (NOT ADDITIONAL_CLEAN_FILES)
      set(ADDITIONAL_CLEAN_FILES "")
    endif ()
    list(APPEND ADDITIONAL_CLEAN_FILES ${WARN})
    set_target_properties(${target} PROPERTIES ADDITIONAL_CLEAN_FILES "${ADDITIONAL_CLEAN_FILES}" TARGET_WARNINGS 1)
    set_target_properties(${target} PROPERTIES TARGET_WARNING_FILES "${WARN}")
    add_dependencies(${target} venv)
  endif ()
endfunction()
...
function(write_warning_files)
  getAllTargets(all_targets)
  message(DEBUG "checking for warning files on ${all_targets}")
  set(WARNING_FILES "")
  foreach (target ${all_targets})
    message(DEBUG "Checking target ${target}")
    get_target_property(TARGET_WARNING_FILES ${target} TARGET_WARNING_FILES)
    if (TARGET_WARNING_FILES)
      list(APPEND WARNING_FILES ${TARGET_WARNING_FILES})
    endif ()
  endforeach ()
  set(WARNINGS_FILE ${CMAKE_CURRENT_BINARY_DIR}/warningfiles.txt)
  message(DEBUG "WARNING_FILES: ${WARNING_FILES}")
  file(GENERATE OUTPUT ${WARNINGS_FILE} CONTENT "$<LIST:JOIN,${WARNING_FILES},\n>")
  add_custom_target(warnings-report COMMAND ${WARNING_COLLECTOR_COMMAND} report ${WARNINGS_FILE})
  add_dependencies(warnings-report venv)
endfunction()
