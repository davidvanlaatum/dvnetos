...
MemBlock *MemPool::findExistingFreeBlock(const size_t size) {
    ...
    while (ptr) {
        if (ptr->size >= size) {
            DEBUG_PRINT(
                "Found block of size " << ptr->size << " for " << size << " bytes in use: " << sizeInUse <<
                " free: " << sizeFree);
    ...
    for (auto i = 0; i <= 1; i++) {
        ...
        if (block != nullptr) {
            ...
            if (block->size > size + memBlockHeaderSize) {
                ...
                DEBUG_PRINT(
                    "Returning " << size << " bytes and adding " << block->size << " bytes back to free list. in use: "
                    << sizeInUse << " free: " << sizeFree);
...
void MemPool::addFreeBlock(MemBlock *block) {
    DEBUG_PRINT(
        "Adding " << block->size << " bytes to free list. in use: " << this->sizeInUse << " free: " << this->sizeFree);
    bool merged;
