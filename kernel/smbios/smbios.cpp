...
namespace smbios {
  ...
  void SMBIOS::dumpTable(const TableHeader *table, const uint16_t length) {
    for (auto ptr = table;
         reinterpret_cast<uint64_t>(ptr) < reinterpret_cast<uint64_t>(table) + length;
...
#define offsetof(member) (reinterpret_cast<uint64_t>(&this->member) - reinterpret_cast<uint64_t>(this))
  ...
  const char *FirmwareInfo::toString(char *buf, size_t size) const {
    static char buf2[32];
    auto n = ksnprintf(buf, size, "Firmware vendor: %s version: %s release: %s rom size: %s",
                       getString(this, this->vendor),
                       getString(this, this->version),
                       getString(this, this->releaseDate),
    ...
    if (this->length == sizeof(FirmwareInfo)) {
      ...
      ksnprintf(buf + n, size - n, " extended Rom: %s",
                bytesToHumanReadable(buf2, sizeof(buf2), bytes));
    }
    return buf;
  }
  ...
  const char *SystemInfo::toString(char *buf, size_t size) const {
    ksnprintf(buf, size,
              "System vendor: %s product: %s version: %s serial: %s sku: %s family: %s",
              getString(this, this->vendor),
              getString(this, this->product),
              getString(this, this->version),
              getString(this, this->serial),
              getString(this, this->sku),
              getString(this, this->family));
    return buf;
  }

  const char *ChassisInfo::toString(char *buf, size_t size) const {
    auto n = ksnprintf(buf, size,
                       "Chassis manufacturer: %s version: %s serial: %s asset: %s",
                       getString(this, this->manufacturer),
                       getString(this, this->version),
                       getString(this, this->serial),
                       getString(this, this->assetTag));
  ...
  const char *ProcessorInfo::getDesignation() const {
    return getString(this, this->socketDesignation);
  }

  ProcessorType ProcessorInfo::getType() const {
    return this->processorType;
  }

  ProcessorFamily ProcessorInfo::getFamily() const {
    return this->processorFamily;
  }

  const char *ProcessorInfo::getManufacturer() const {
    return getString(this, this->processorManufacturer);
  }

  uint64_t ProcessorInfo::getId() const {
    return this->processorId;
  }

  const char *ProcessorInfo::getVersion() const {
    return getString(this, this->processorVersion);
  }

  uint16_t ProcessorInfo::getExternalClock() const {
    return this->externalClock;
  }

  uint16_t ProcessorInfo::getMaxSpeed() const {
    return this->maxSpeed;
  }

  uint16_t ProcessorInfo::getCurrentSpeed() const {
    return this->currentSpeed;
  }

  bool ProcessorInfo::isPopulated() const {
    return this->status & 1 << 6;
  }

  ProcessorStatus ProcessorInfo::getStatus() const {
    return static_cast<ProcessorStatus>(this->status & 0x7);
  }
  ...
  const char *PhysicalMemoryArray::toString(char *buf, size_t size) const {
    auto n = ksnprintf(buf, size, "Physical Memory Array handle: %x location: %s use: %s error: %s",
                       this->handle,
                       ::smbios::toString(this->location),
                       ::smbios::toString(this->use),
